...
const pokemonObject = JSON.parse(fetchPokemonData());

const body = document.querySelector('body');
body.setAttribute('style', 'display: flex; justify-content: space-around;gap: 10px; flex-wrap: wrap;');

Object.entries(pokemonObject.sprites)
  .filter(item => item[1] !== null)
  .forEach(item => createCard(item));

function createCard(item) {
  const card = document.createElement('div');
  card.setAttribute('style', 'background-color: khaki; border-radius: 3px; text-align: center;');
  body.append(card);
